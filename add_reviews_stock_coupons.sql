...
CREATE TABLE product_reviews (
  ...
  order_id    UUID REFERENCES orders(id) ON DELETE SET NULL,  -- Solo compras verificadas
  rating      SMALLINT NOT NULL CHECK (rating BETWEEN 1 AND 5),
  title       VARCHAR(150),
  body        TEXT,
  is_verified BOOLEAN DEFAULT FALSE,   -- TRUE si el user compró el producto
  is_approved BOOLEAN DEFAULT TRUE,    -- Para moderación por admin
...
-- Columnas de caché de valoración en products (para rendimiento)
ALTER TABLE products ADD COLUMN IF NOT EXISTS review_count   INTEGER DEFAULT 0;
ALTER TABLE products ADD COLUMN IF NOT EXISTS rating_avg     NUMERIC(3,2) DEFAULT 0;

-- Función que actualiza el caché automáticamente
...
-- ============================================================
-- 2. STOCK VISIBLE (ya existe la columna stock en products)
-- Los umbrales de alerta se configuran por entorno, pero
-- añadimos una vista útil para el frontend.
-- ============================================================
...
SELECT
  ...
  CASE
    ...
    WHEN stock <= 3       THEN 'pocas_unidades'   -- "¡Solo quedan X!"
  ...
  CASE
    ...
    ELSE FALSE
  END AS show_stock_count   -- El frontend muestra el número solo si es TRUE
FROM products
...
CREATE TABLE coupons (
  id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  code            VARCHAR(50) UNIQUE NOT NULL,          -- 'VERANO10', 'BIENVENIDA'
  description     VARCHAR(255),
  discount_type   VARCHAR(20) NOT NULL                  -- 'percent' | 'fixed'
                  CHECK (discount_type IN ('percent','fixed')),
  discount_value  NUMERIC(10,2) NOT NULL CHECK (discount_value > 0),
  -- Restricciones de uso
  min_order_eur   NUMERIC(10,2) DEFAULT 0,              -- Pedido mínimo para aplicar
...
-- Log de uso de cupones (para controlar max_uses_per_user)
...
-- Columna en orders para almacenar el cupón aplicado
...
-- Cupones de ejemplo
INSERT INTO coupons (code, description, discount_type, discount_value, min_order_eur, max_uses, valid_until) VALUES
  ('BIENVENIDA10', '10% descuento en tu primer pedido', 'percent', 10,  0,    NULL, NOW() + INTERVAL '1 year'),
  ('VERANO5',      '5 EUR de descuento en verano',       'fixed',    5,  30,   500,  '2025-09-30 23:59:59'),
